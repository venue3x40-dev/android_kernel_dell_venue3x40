...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (strlen(firmware_path) == 0 || (strlen(fw_path) && fw_path[strlen(fw_path) -1] == '/')) {
		fw_auto_name = 1;
...
#endif /* ARP_OFFLOAD_SUPPORT */
	//register_inet6addr_notifier(&dhd_notifier_ipv6); /* david */
...
{       //patch for "wifi can't connect at HT40 2.4G" from liangt@broadcom.com 20140321
            /* HT40/20 setting */
            uint32 mimo_bw_cap = 1;
            bcm_mkiovar("mimo_bw_cap", (char *)&mimo_bw_cap, 4, iovbuf, sizeof(iovbuf));
            if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
                sizeof(iovbuf), TRUE, 0)) < 0) {
                DHD_ERROR(("%s mimo_bw_cap failed  %d\n", __FUNCTION__, ret));
            }
            DHD_ERROR(("%s mimo_bw_cap set at end\n", __FUNCTION__));
}
...
#endif /* ARP_OFFLOAD_SUPPORT */
	//unregister_inet6addr_notifier(&dhd_notifier_ipv6); /* david */
