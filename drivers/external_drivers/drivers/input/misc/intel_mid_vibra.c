...
#if 1
static struct vibra_info *pwm_info;
#endif
...
#if 1
int bl_pwm_configure(u8 duty_val, int level)
{
	struct vibra_info *info;
	union sst_pwmctrl_reg pwmctrl;
	
	if (!pwm_info) {
		printk(KERN_INFO "%s error* : soc pwm is not configure\n", __func__);
		return 0;
	}
...
//		pr_debug("Read pwmctrl %x\n", pwmctrl.full);
		...
		if (pwmctrl.part.pwmswupdate){
			printk("check pwnswupdate bit fails\n");
...
EXPORT_SYMBOL(bl_pwm_configure);
#endif
...
{
	//Fix backlight issue patch from bug:344
	return 0;
#if 0
			...
			const struct pci_device_id *pci_id)
{
	...
	(pci->resource[0]).start = 0xff013000;
	/* vibra Shim */
